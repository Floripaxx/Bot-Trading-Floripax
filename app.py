...
class MexcTradingBot:
    ...
    def get_real_btc_price_mexc(self):
        """SOLO ESTO CAMBIÉ: Obtiene precio REAL de BTC desde MEXC"""
        try:
    ...
    def mexc_api_request(self, endpoint, params=None, method='GET'):
        ...
        if self.demo_mode:
            ...
            if 'ticker' in endpoint:
                # SOLO ESTO CAMBIÉ: Precio REAL desde MEXC
                real_price = self.get_real_btc_price_mexc()
                return {'price': str(real_price)}
    ...
    def check_positions_for_close(self, current_price):
        ...
        for position in self.open_positions:
            ...
            if abs(pnl) > position['quantity'] * position['price'] * 0.003:
...
def main():
    ...
    if st.session_state.running:
        # SOLO ESTO CAMBIÉ: Obtener precio REAL de MEXC
        btc_price = bot.get_real_btc_price_mexc()
