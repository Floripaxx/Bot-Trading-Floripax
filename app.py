...
class MexcFuturesTradingBot:
    ...
    def execute_trade(self, action: str, price: float):
        ...
        try:
            # ========== NUEVO: TAMAÑO DINÁMICO CON INTERÉS COMPUESTO ==========
            ...
            if action in ['buy', 'sell'] and self.open_positions < self.max_positions:
                # ABRIR POSICIÓN
                investment_amount = self.cash_balance * dynamic_position_size * self.leverage
                ...
                # ========== NUEVO: MOSTRAR TAMAÑO DINÁMICO EN LOG ==========
                trade_info = f"{side_emoji} TURBO {self.position_side.upper()} {self.symbol}: {quantity:.6f} @ ${price:.2f} | Size: {dynamic_position_size*100:.1f}% | Margen: ${investment_amount/self.leverage:.2f} | Leverage: {self.leverage}x"
